...
/** Promote a co-borrower to primary: swap borrower_id, demote the old primary to co. */
export async function promoteToPrimary(sb: SupabaseClient, dealId: string, contactId: string): Promise<void> {
  const { data: deal } = await sb.from('deals').select('borrower_id').eq('id', dealId).maybeSingle()
  const oldPrimary = (deal as { borrower_id: string | null } | null)?.borrower_id ?? null
  if (oldPrimary === contactId) return // already primary

  const { error: upErr } = await sb.from('deals').update({ borrower_id: contactId }).eq('id', dealId)
  if (upErr) throw new Error(`promoteToPrimary set borrower_id failed: ${upErr.message}`)
