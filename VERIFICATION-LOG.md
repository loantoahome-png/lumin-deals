...
**Deployed:** commit `181967c` → dpl `r0zzjfqv5` (Ready, 47s), prod alias confirmed resolving to it via `vercel inspect`. ⚠️ **The Vercel CLI reported false failures today:** 4× `vercel --prod` all exited `Error: ... v13/deployments ... read ETIMEDOUT` while the upload actually SUCCEEDED and the deployment was created server-side each time — the CLI just never got the response. Result: 4 redundant builds of the same commit. **Check `vercel ls` before retrying a "failed" deploy** (and note the `build-passed`/`vercel-deploy succeeded` hook fires on the command, not the outcome — don't trust it as a result signal).
