...
**Status:** CHANGED (code complete + committed) — **NOT deployed; awaiting Randy's GHL Location ID + email to set env.** tsc 7-baseline / **0 new** across 19 changed files; `npm run build` READY; fixtures **cohort 74/74 + lead-report 63/63**.
...
**Remaining (env + deploy — awaiting Efrain):** `.env.local` + **Vercel prod**: `GHL_API_KEY_2`=pit-18d2a767-… (have), `GHL_LOCATION_ID_2` + `NEXT_PUBLIC_GHL_LOCATION_ID_2`=‹Randy loc id› (NEED), `LO_EMAIL_RANDY`=‹email› (NEED — lock-alerts only), optional `TASK_ASSIGNEE_EMAILS` add "Randy Mathis". NEXT_PUBLIC is build-inlined → set Vercel env BEFORE `vercel --prod`, then trigger a sync and confirm Randy's deals land with correct LO attribution.
