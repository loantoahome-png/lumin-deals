...
**Status:** CHANGED — DDL applied to prod (Management API, tables `import_runs` 17 cols / `import_changes` 9 cols confirmed); writer exercised with a synthetic run (deleted after); history page browser-checked on prod. Not yet exercised by a REAL commit — the next applied import is the proof.
