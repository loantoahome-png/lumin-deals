...
**Status:** **CHANGED** — code complete, fixtures 57/57, tsc clean (no new errors), `next build` ✓. **BLOCKED on one manual step:** `supabase-calls.sql` must be run in the Supabase SQL editor; the `calls` table does not exist yet (verified: `Could not find the table 'public.calls'`). Until then `/calls` renders its "run the SQL first" state and nothing can be imported.
...
**Result:** *(pending — awaiting the SQL step)*. Live check asserts the pre-measured figures: 7,348 rows · re-import adds 0 · Moe 717 leads/93% dialed/87% connected/$2,079 never-dialed · Matt 626/96%/89%/$762 · median first dial 24 min · LMB $38, OwnUp $80, Lending Tree $47, Lendgo $26, FRU $32 per connect · no dial >1h before its lead-in date · Randy absent.
