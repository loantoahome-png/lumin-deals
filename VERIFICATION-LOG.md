...
**Changes:** [components/SourceStateBreakdown.tsx](components/SourceStateBreakdown.tsx) — **new**, extracted out of `app/lead-roi/page.tsx` (past 1,600 lines) so it can be rendered in isolation, which is the only way to check it locally. Source tabs with lead counts; above `MAX_TABS = 6` the tail rolls into a **More (N)** dropdown, and a source picked from it pins into the strip so the active tab is never hidden. [lib/leadRoi.ts](lib/leadRoi.ts) — `StateStats` extended to the FULL source-table column set (cold, optout/orate, teamRemoved/trate, open, active, lost, fundedAvg); the open/active/lost else-chain is a deliberate copy of `buildSourceStats` so the buckets reconcile. [app/lead-roi/page.tsx](app/lead-roi/page.tsx) — renders the section; `stateTabPick` is clamped through `stateTabSource` so switching LO or narrowing the source filter can't strand the selection on a missing tab. [app/lead-roi/report/page.tsx](app/lead-roi/report/page.tsx) — new **By source and state** section: a PDF has no tabs, so every source gets its own table, stacked, with `break-inside: avoid`. [scripts/lead-roi-check.ts](scripts/lead-roi-check.ts) — 12 new fixtures incl. `open+active+lost+funded = leads` per state and bucket-parity with the source row.
**⚠️ Bug caught in verification, would have shipped:** the section card had `overflow-hidden` (copied from the matrix card), which **clipped the "More" dropdown**. The menu rendered with a real 220×178 box and was invisible — a dead button, in exactly the 37-source case the dropdown exists for. Found by reading `getBoundingClientRect()` + computed style off the live DOM after the screenshot looked wrong. Fixed by dropping `overflow-hidden` and rounding the header/footer edges instead; a comment in the component says why it must not come back.
**Test Method:** `/lead-roi` → below the matrix, "One source, every state"; click a tab, then **More (N)** under All sources. `npx tsx scripts/lead-roi-state-report.ts` must print `reconcile → leads OK · spend OK · net OK · submitted OK · applied OK · open OK · active OK · lost OK · optout OK · volume OK`. Visual Report → the By source and state section.
**Result:** VERIFIED interactively on synthetic data (tab switch, overflow dropdown open, dropdown selection pinning into the strip, header + footer totals updating); reconciliation VERIFIED on live data for all four LOs. Prod look still pending Efrain.
...
**Changes:** [lib/leadRoi.ts](lib/leadRoi.ts) — `SUBMISSION_RULE` → `'status_only'`; `isSubmittedUnder` keeps an `'application'` branch (the file clause, 345 · 6.6%) for a future App % column; the third `dead_file_or_status` reading was **removed**, since its premise was that a file implies submission. New `SUBMISSION_DESC` derives the UI copy from the rule so a label can't drift from the arithmetic. [app/lead-roi/page.tsx](app/lead-roi/page.tsx) + [app/lead-roi/report/page.tsx](app/lead-roi/report/page.tsx) — every tooltip claiming the Arive clause rewritten; the Sub % header now states the rate is a **floor** (a loan submitted then declined reads Not-Ready and is unrecoverable). [scripts/lead-roi-check.ts](scripts/lead-roi-check.ts) — 4 explicit guards that an Arive file does NOT count, plus both rules pinned side by side. [scripts/lead-roi-state-report.ts](scripts/lead-roi-state-report.ts) — reports submissions and applications as separate lines.
**Test Method:** `npx tsx scripts/lead-roi-state-report.ts` — header must read `submitted to UW (the live metric): 108 (2.1%)` and `rule in force: status_only`.
**Result:** VERIFIED. Repo-wide submitted = **108 of 5,225 priced (2.1%)**; applications = 345 (6.6%), reported separately. Per-LO: Moe 33 (1.9%), and per-source Sub % now sits just above Fund % (Lendgo 1.3% vs 1.1%, LMB 2.6% vs 2.1%, OwnUp 4.4% vs 3.1%) — correct, but far less discriminating than the application rate. **Offered to Efrain:** an App % column off the already-pinned `'application'` rule, where vendors actually separate (OwnUp 13.1% / LMB 7.0% / Lendgo 3.3%).

### [2026-09-21] /lead-roi — per-source state breakdown + Submission %
**Status:** CHANGED — tsc = the 7-error `main` baseline (0 new), `npm run build` ✓ exit 0, **139/139** fixtures (`lead-roi-check` was 98), live numbers verified service-role via the new `scripts/lead-roi-state-report.ts`, page renders with no console errors on the bypass dev server (data empty there — `deals` RLS rejects anon reads, as documented).
**Issue:** Efrain: "look at metrics from the source and break it down between all different states we received leads from for that source… also add submission percentage for the leads."
**Changes:** [lib/leadRoi.ts](lib/leadRoi.ts) — `isSubmitted` / `isSubmittedUnder` + the `SUBMISSION_RULE` switch; `submitted`/`sr` on `SourceStats`, `RoiKpis` and `StateRow`; a **Submitted** funnel stage; new `stateStats()` (full money set per state, retainer split pro-rata by lead count with the last state absorbing the remainder so Σ spend reconciles EXACTLY) and `sourceStateMatrix()`. [app/lead-roi/page.tsx](app/lead-roi/page.tsx) — `arive_file_no` added to `LEAD_COLS`; Sub % column + totals; per-source states table in the drill-down; a Source × State matrix with an 8-metric picker; a Submitted KPI; CSV gains `Submitted` + `Sub %`. [app/lead-roi/report/page.tsx](app/lead-roi/report/page.tsx) — its OWN `LEAD_COLS` copy got `arive_file_no` too, plus Sub % in the table, tfoot and Per state. [scripts/lead-roi-check.ts](scripts/lead-roi-check.ts) — 41 new fixtures. [scripts/lead-roi-state-report.ts](scripts/lead-roi-state-report.ts) — new read-only live verifier.
**Test Method:** `npx tsx scripts/lead-roi-state-report.ts "Moe Sefati"` — the source table must match the page (Lendgo 627 leads / $14,760 spend / $19,994 net rev / 1.35× ROI), and every drill-down must print `reconcile → leads OK · spend OK · net OK · submitted OK`. On prod: expand a source → the by-state table; the Source × State matrix below the table.
**Result:** VERIFIED (math + reconciliation). Repo-wide submitted = **345 of 5,222 priced leads (6.6%)** — 108 from the status rank, 237 from the Arive-file clause. ⚠️ **Open with Efrain:** 162 of those 237 are still in the Leads group (151 `App Intake`), i.e. a file was *opened* in the LOS, not necessarily sent to underwriting. If an Arive file number is issued at application, the metric's honest label is "application taken" — flip `SUBMISSION_RULE` to `dead_file_or_status` (182 · 3.5%) or `status_only` (108 · 2.1%); all three branches are fixture-pinned.
...
**Changes:** [lib/lockStatus.ts](lib/lockStatus.ts) (new) — the single chokepoint: `lockStatus()` (`locked` / `expiring` / `expired` / `no-expiry` / `unlocked` + `needsLock`), `lockDaysLeft()`, `unlockedEscrows()`, `lockCounts()`. **`lock_expiration` decides; the `locked` flag does not** — measured the same day, `locked = 'Yes'` on **0 of 32** active escrows while 25 carry a real Arive expiry, so gating on the flag reports all 32 as unlocked. Funded is excluded by scope because the `clear_lock_expiration_on_funded` trigger nulls the column (0 of 136 funded rows carry one). Date-only column parsed as LOCAL midnight. [components/Dashboard.tsx](components/Dashboard.tsx) — `locked,lock_expiration` added to `DASHBOARD_COLS`; new card between the Tasks widget and the charts row, in the existing `CARD`/`CardHeader`/`Pill`/`ROW` system: red stripe = expired (longest first), amber = never locked (deepest stage first), header counters `N expired` / `N never locked` / `N of M escrows locked` / `N expiring ≤7d`, green confirmation when all are locked, and a `+ N more under loan officers not selected above` footer so the Matt+Moe default filter can't hide Randy's and Daniel's (today that reads **+ 7**). [scripts/lock-status-check.ts](scripts/lock-status-check.ts) (new) — 17 offline fixtures. [scripts/unlocked-loans-report.ts](scripts/unlocked-loans-report.ts) (new) — live service-role report of exactly what the card renders.
**Test Method:** logged in, open `/` → the card sits under "Tasks · overdue & today"; with all four LOs checked it reads `3 expired · 7 never locked · 22 of 32 escrows locked`; with the default Matt+Moe it reads `3 never locked · 22 of 25` plus `+ 7 more under loan officers not selected above`. Cross-check any time with `npx tsx scripts/unlocked-loans-report.ts`.
**Result:** (pending Efrain's look at prod)

### [2026-09-09] Dashboard + shared chrome restyle — flat surfaces, color by meaning (v2 mockup implemented)
**Status:** CHANGED — verified locally: `npx tsc --noEmit` = the 7-error `main` baseline (none in touched files), eslint = per-file HEAD baseline (6 pre-existing `react-hooks/set-state-in-effect` findings on untouched lines), 30/30 offline fixtures, `npm run build` ✓, bypass dev-server screenshots of `/`, `/deals`, `/tasks` with no console errors. Deal metrics render $0 under the bypass (`deals` RLS), so the populated stat strip / stage chart / Next-steps table still need Efrain's look at prod.
**Issue:** The dashboard read as an admin template: five competing hues above the fold (orange verse card, blue gradient hero, green/violet/amber KPI accents that collided with Matt/Randy/Moe's LO colors one card below), gradient chart bars and stacked shadows, two card recipes and three header treatments on one page, a rainbow donut for a 3/2/2/1/1 loan-type split, and the system font with no tabular figures. Efrain approved the v2 mockup (https://claude.ai/code/artifact/f3dcac6f-916e-4305-a73a-db442bde300f) on 2026-09-09; v1 (blue-only) had been rejected as too monochrome.
**Changes:** [components/Dashboard.tsx](components/Dashboard.tsx) — JSX rewrite, data logic byte-for-byte: header band (title, scope line, `updated` stamp, LO chips), one four-cell stat strip (solid blue-700 hero + tinted-badge tiles + a stage-mix strip in stage colors), one card/header/pill/row system, stage chart bars in `STATUS_STRONG` with a "Funding soon" bracket, loan types as sorted bars in `LOAN_TYPE_COLORS` (donut removed), LO performance sorted by volume with share, quiet "Nothing missing" state, Next-steps table (stage pill via `STATUS_COLORS`, owner LO dot, due column, "N without a next step" chip). [lib/types.ts](lib/types.ts) — `STATUS_STRONG` (solid tones of the pill hues) + `LOAN_TYPE_COLORS` (fixed per type). [app/layout.tsx](app/layout.tsx) + [app/globals.css](app/globals.css) — Public Sans + Geist Mono via `next/font`, wired into Tailwind with `@theme inline` so `font-sans` is the default and `font-mono` = Geist Mono; ground `#f4f6fb`. [components/Sidebar.tsx](components/Sidebar.tsx) — navy `#0f1a38`, "L" monogram tile, quieter items (blue-600 active kept), condensed footer (sync badge + smaller Sync button + one identity/sign-out row; © line removed); [components/GlobalSearch.tsx](components/GlobalSearch.tsx) + [components/NotificationBell.tsx](components/NotificationBell.tsx) restyled to the rail. [components/LoFilter.tsx](components/LoFilter.tsx) — dot chips; a selected chip fills with that LO's tint (`LO_TINT`) — app-wide. [components/DailyVerse.tsx](components/DailyVerse.tsx) + [app/page.tsx](app/page.tsx) — one-line orange strip above the page instead of a boxed card. [components/UnreadInbox.tsx](components/UnreadInbox.tsx) — flat rows (nested cards gone), badge header, blue unread pill, deal-stage pill via `STATUS_COLORS`, mono timestamp. [components/TaskBoard.tsx](components/TaskBoard.tsx) — the `violet` due tone now renders blue (violet is Randy's LO color); `/tasks` "Today" labels change color too, nothing else.
**Test Method:** logged in, open `/`: stat strip shows volume / funding soon / escrows / avg with the stage-mix strip under the volume; stage bars match the pill colors on Pipeline; loan types as bars; Next steps rows show colored stage pills + owner dots; sidebar navy with the blue active item; verse as a strip. `/deals` and `/lead-roi`: selected LO chips tinted. `/tasks`: "Today" items read blue, "Overdue" red.
**Result:** (pending Efrain's look at prod)

### [2026-09-02] Arive import — status + loan_amount are GHL-OWNED until funded (skipped, never written)
**Status:** VERIFIED (preview, prod) — Efrain uploaded a fresh export (`DB Import - 2026-09-02T22_20_49.903Z.csv`) at 15:20 PT: 1 fill + 4 overwrites (Mark Blackstone ltv/purchase_price/estimated_value/credit_score; Andrea Walsh `Broker Check Received → Loan Finalized`, a FUNDED deal — the one case status still writes), GHL-owned 0 (the DB still equals the 22:14 import, so nothing had drifted yet). Confirmed field-by-field with the plan builder against the live DB. Not applied by choice. 29/29 fixtures, lint + tsc clean, build passes, prod `91b4b2c`.
**Issue:** Efrain re-exported Arive 85 min after applying an overwrite import and the preview showed **334 overwrites again** (status 198, loan_amount 119) although Arive had changed 4 loans. The import log (above) + `updated_at` proved it: the 21:30Z maintenance sync pass (full rescan) wrote GHL's stage and opportunity value back onto every non-funded deal the import had touched. By design the sync writes `status` on every deal and `loan_amount` = GHL opp value on every non-funded deal, so an overwrite import and the sync were fighting, the sync always won within the hour, and the same ~300 "overwrites" reappeared on every preview.
**Changes:** [lib/ariveCsv.ts](lib/ariveCsv.ts) — `GHL_OWNED_FIELDS` (`status`, `loan_amount`) + a new `FieldChange.action` `'ghl_owned'`: when the deal is not funded AND this row does not fund it, a differing value in either field is surfaced as `ghl_owned` and never written (fill or overwrite); `summarizePlan` reports `fields_ghl_owned`. Funded deals, and rows that fund a loan, write both fields exactly as before. [app/import/arive/page.tsx](app/import/arive/page.tsx) — "GHL-owned · skipped" tile, a GHL-owned row filter, greyed diff rows with a tooltip saying why, and the safety note spells out the rule. [scripts/arive-match-check.ts](scripts/arive-match-check.ts) — 9 fixtures (lead skipped, blank amount not even filled, row that funds writes both, funded deal keeps Arive).
**Test Method:** re-upload any Arive export in overwrite mode → the tile shows a handful of real Arive changes and the GHL-owned tile absorbs the ~300; apply → the import-log run for it has no `status`/`loan_amount` rows on non-funded deals; a funded loan (e.g. one with a `Loan Funded` stage in the CSV) still writes both.
**Result:** preview verified as above; the GHL-owned tile will only show its ~300 once the next maintenance sync pass re-drifts status/loan_amount.

### [2026-09-02] Arive import — per-field IMPORT LOG (what did that import change?)
**Status:** VERIFIED — first real commit logged at 22:14Z the same day (`DB Import - 2026-09-02T22_10_20.144Z.csv`, overwrite mode: 290 deals, 534 fields = 198 status + 198 pipeline_group + 119 loan_amount + 19 others), and that log is what proved the GHL-sync ping-pong below. DDL applied to prod via the Management API; synthetic run deleted; history page browser-checked on prod.
**Issue:** Efrain asked whether "Will fill blanks 132 / Will overwrite 444" was exaggerated. It was answerable only in part (see `docs/diagnoses/2026-09-02-import-counts-diagnosis.md`): `deals` keeps no history, previews aren't persisted, and the change-log CSV had not been downloaded, so ~300 of the 444 overwrites could not be attributed field by field.
**Changes:** NEW [supabase-import-log.sql](supabase-import-log.sql) — `import_runs` (one row per applied import: file, mode, shields, counts split fill/overwrite/create) + `import_changes` (one row per field written: deal, field, old → new, action), RLS-on/no-policies like `stage_events` · NEW [lib/importLog.ts](lib/importLog.ts) — `recordImportRun` (chunked inserts, non-fatal) · [app/api/import/arive/route.ts](app/api/import/arive/route.ts) — collects every field actually written (incl. the derived `pipeline_group`, logged against the prior stored value; shielded fields never appear because they are never written) and persists once at the end; response carries `run_id` · NEW [app/api/import/arive/history/route.ts](app/api/import/arive/history/route.ts) — runs list / one run's changes / one deal's import history · NEW [app/import/arive/history/page.tsx](app/import/arive/history/page.tsx) — runs table → per-field chips (click to filter) → per-deal old→new list, search, action filter, CSV · [app/import/arive/page.tsx](app/import/arive/page.tsx) — sends `filename`, header link, post-commit "View every field this import wrote" (or a warning if the log did not save) · sidebar "Import History".
**Design decisions:** preview never logs (only commits) · values stored as TEXT via `valueText` so every column type fits one table · logging failure never fails the import (the deals are already written) · admin-only by the existing `/import` prefix rules.
**Test Method:** apply the next Arive import → the completion card links to the run → Import History shows the same fill/overwrite totals as the tiles, and clicking `loan_amount` lists exactly the deals it changed.
**Result:** pending the next real import.

### [2026-08-31] GHL task board — edit the DUE DATE, not just the owner
**Status:** VERIFIED end-to-end against live GHL (write path); UI render not browser-verified (`ghl_tasks` RLS blocks the auth-bypass dev server — see the deals-rls note).
**Issue:** Efrain asked whether pushing a GHL task's date out instead of clearing it would update on the board. It does (the sweep re-fetches every open task and upserts `due_at`) — but it could only be done **in GHL by hand**. The dashboard offered complete, delete and reassign, so the nearest in-app action to "not done yet, push it out" was to COMPLETE the task, which is a lie and drops the row off the board entirely: the mirror holds open tasks only, so a completed task is buried in GHL's history. `deal_tasks` rows have had inline due-date editing all along; mirrored rows were the gap.

**Changes:** NEW [app/api/ghl/tasks/reschedule/route.ts](app/api/ghl/tasks/reschedule/route.ts) — `POST { taskId, dueDate }` → `PUT /contacts/{cid}/tasks/{id} { dueDate }`, reading OUR stored row for the location/contact (same no-trust-the-client rule as complete/delete/reassign), confirming via the single-task GET and updating the mirror so the card re-buckets immediately · `normalizeDueDate`, `sameDueDate` and `rescheduleGhlTask` in [lib/ghlTasks.ts](lib/ghlTasks.ts) · [components/GhlReassign.tsx](components/GhlReassign.tsx) **renamed** to [components/GhlTaskEdit.tsx](components/GhlTaskEdit.tsx) — due date + time + `+1 day / +3 days / +1 week` push-out chips alongside the existing owner picker · rewired on all three surfaces (`/tasks`, the Follow-Up cockpits, the deal card), state renamed `reassigning` → `editingGhl` · 25 new fixtures in [scripts/ghl-tasks-check.ts](scripts/ghl-tasks-check.ts) · NEW [scripts/ghl-reschedule-live-check.ts](scripts/ghl-reschedule-live-check.ts).

**Design decisions:**
- **One call per CHANGED field**, not a single PUT of everything. GHL's update takes a partial body, so an unchanged field is never sent — which is what keeps a reschedule from touching the owner and vice versa.
- **⚠️ Push-out chips shift from the LATER of the form's date and today.** "+3 days" on a task 30 days overdue must land three days from *now*; shifting from the stale date would leave it still overdue, defeating the entire point.
- **The date cannot be cleared.** A GHL task cannot be undated (POST without one is a 422), so the field is required and `normalizeDueDate` refuses empty/junk input **before** any request — an empty `dueDate` on a PUT is the same family as `{title:''}`, which returns **200 and blanks the field**.
- **⚠️ Half-saves are handled, not hidden.** If the owner lands and the date fails, the panel stays open reporting exactly that, a retry only redoes the failed half, and closing still hands the parent the half that worked rather than leaving a stale card.
- Still **no title/description edit** — that text lives in GHL.

**⚠️ `sameDueDate` compares epoch ms, never strings.** GHL echoes a date back in its own formatting, so `2027-06-16T15:00:00.000Z` and `2027-06-16T15:00:00Z` are one instant written two ways; a string compare would report a write that landed perfectly as a failure — the same misread that once made a working reopen look like a no-op.

**Test Method:** `npx tsx scripts/ghl-tasks-check.ts` · `npx tsx scripts/ghl-reschedule-live-check.ts` (creates a throwaway task in the primary sub-account, reschedules it, then deletes it — no real task is touched) · `npx tsc --noEmit` · `npx eslint` · `npm run build`.
**Result:** fixtures **87 passed / 0 failed** (was 62). Live check **8 passed / 0 failed**: `PUT { dueDate }` alone is accepted, the **single-task GET confirms the new date**, and the partial body left the title, the owner and the open/completed flag untouched — the claim the date-only action rests on. Cleanup proven by re-reading the task (400 "The task id is invalid."), not by the DELETE's own 200. tsc error set unchanged from main (7, all pre-existing in reports/underwriting/DealForm/next.config); GhlTaskEdit.tsx is eslint-clean — the `set-state-in-effect` warning inherited from GhlReassign was fixed by deriving `taskId` instead of setting error state in the effect. Build compiles, `/api/ghl/tasks/reschedule` registered. **Still needs Efrain's eyes on the rendered panel.**
...
**Tasks.** The follow-up date/time picker is gone from the Next Step block; in its place the card lists the loan's **open tasks** (title, due chip, assignee, `GHL` badge on mirrored rows, first 3 + "N more") and an **Add task** button that opens the shared `DealTasks` panel in a modal, straight into the create form. Same `deal_tasks` rows as /tasks and the processing desk, plus this contact's mirrored GHL tasks — no second task system. Tasks are fetched **once per board** (paginated past the 1000-row PostgREST cap), not per card.
**⚠️ The date filters had to follow.** `Overdue` / `Today` / `This week` (chips, counts and the card's red/violet ring) now read **the earlier of `next_action_due` and the soonest open task**. Left alone they'd have counted 0 next to cards ringed red, since the only place to set `next_action_due` from this board was the picker that was removed.

**⚠️ Lock — the expiry DATE is the evidence, not the `locked` flag.** My first pass gated on `locked === 'Yes'` (what `/reports/escrows` does). Checking real rows killed it: **12 of 18 active escrows carry an Arive lock expiry, only 2 have `locked = 'Yes'`** (Tommy Moua sits at Docs Signed with a Sep 4 expiry and `locked = 'No'`). `lock_expiration` is imported from Arive (`lib/ariveCsv.ts`); **`locked` has no import mapping at all** — it is hand-set on the deal page, defaults to `'No'`, and goes stale immediately. Gating on it would have printed "Not locked" over 10 live locks and silenced the expiry alert the card has always shown. Dates are parsed as **local** midnight (`lock_expiration` is a DATE column; bare `new Date()` reads a day early in Pacific).
**Priority.** The High/Normal/Low boxes and the "Marked as high priority" chip are gone. A high-priority file still announces itself as a flame in the alerts row; set priority on the deal page, Pipeline, or the processing desk.
**Changes:** [components/EscrowTracker.tsx](components/EscrowTracker.tsx) (lock summary, task panel + modal, task-aware filters, board-level task fetch; `FollowUpPicker`/`PresetButton`/`PRIORITY_OPTIONS` deleted), [components/DealTasks.tsx](components/DealTasks.tsx) (`startAdding`, `onChanged` props).
**Test Method:** `npx tsx scratchpad/_probe-escrow-card.ts` for the data path (the board renders empty under the local auth-bypass — `deals` RLS rejects anon); a throwaway harness page fed EscrowTracker fixture deals carrying **real deal ids**, so the component's own task fetch matched them.
**Result:** `next build` ✓, `tsc` = the same pre-existing errors, none in a touched file. In the browser: David Kerscher's card showed his 2 real overdue `deal_tasks` with "Overdue 13d"/"Overdue 6d" and a red ring; the Overdue chip read **1** and filtered to exactly that card; lock rendered green (`Sep 11 · 24d left`, Rugley), amber (`Aug 22 · 4d left`), red (`Expired Aug 12 · 6d ago`) and slate (`Not locked`); the Add-task modal opened on the create form with both existing tasks listed. **No task was created — the local dev server points at the production database and creating one emails the assignee.**
**Still unverified:** the GHL half of the list. `ghl_tasks` is RLS-blocked for anon, so mirrored rows render only when signed in (`deal_tasks` reads fine either way). Same client and query `DealTasks` already uses live on /tasks, but the escrow card has not been seen with a GHL row in it.

### [2026-08-12] /calls — per-dialer filter, and a dedupe hole that was double-counting Brianne
**Status:** **VERIFIED.** 30/30 suites (`calls-check` 74 → **92**, `calls-api-check` 41 → **44**), `tsc` = the same 7 pre-existing errors, `next build` ✓, exercised in the browser, deployed `lumin-deals-izi3559by` ● Ready.
**Issue:** Efrain: *"this page's main focus is to see how many calls Brianne is doing since she is the lead caller"* → *"Can we add a filter so we can filter it per person?"*

**Filter.** "Dialer" chips sit beside the date presets and drive **everything** on the Activity tab — KPIs, outbound/inbound, best hour, daily trend, hour and weekday charts. `DayBucket`/`HourBucket` gained `dialer` and `DialerDayBucket` gained `account`, so it still recomputes **client-side with no refetch** (the property that makes every preset instant). Selecting a person turns the dialer table into their **per-sub-account split** — the dialer label merges a person's two numbers, so that question was previously unanswerable on the page. Verified live: Brianne 736 this month = **Matt Park 391 + Moe Sefati 345**, totalling exactly the KPI; unfiltered, the five dialers partition 1,166 exactly.
**⚠️ `activeDays` now counts DISTINCT days.** Buckets are per day PER DIALER, so counting rows would have deflated "calls / active day" ~4×. Fixture-locked, and confirmed in the browser (everyone = 10 active days, Brianne = 8).
**⚠️ Three of my own fixture expectations were wrong and the fixtures caught them** — Matthew's only test call is INBOUND, and inbound is excluded from the hour analysis by design. Corrected the assertions, not the code.

**⚠️⚠️ THE REAL FIND — a dedupe hole, spotted in Efrain's screenshot, not by a test.** His screenshot showed an **"Unknown" dialer with 2 calls**. Those calls had **no dialing number at all**: GHL returned them just after they finished, with `from`/`to` empty. They stored with `dialer_number_phone = NULL`, a later fetch returned them complete, and **both inserted a second time** — because **Postgres treats NULL as DISTINCT in a unique index**, so `calls_dedupe_uniq` could never match them. The earlier "replaying a window inserts 0" verification was true and still missed this, because it only holds while every key column is non-null. Impact landed precisely on the page's core metric: Brianne double-counted, plus a phantom dialer. **Fixes:** the mapper now writes `''` never NULL, and the sweep holds its right edge back **5 minutes** (`SETTLE_MS`) so a call is only read once GHL has settled it. The 2 duplicate rows were deleted after verifying each had a labelled twin at the same second with the same duration. Post-fix: "Unknown" is gone from the page.
**Changes:** [lib/callsReport.ts](lib/callsReport.ts) (dialer-keyed buckets, `accountsForDialer`, `dialerNamesInRange`, distinct-day counting), [app/calls/page.tsx](app/calls/page.tsx) (filter chips, per-account table, summed trend), [lib/callsApi.ts](lib/callsApi.ts) (`''` dialer), [lib/callsSync.ts](lib/callsSync.ts) (`SETTLE_MS`), fixtures in both check scripts.
**Test Method:** `npx tsx scripts/calls-check.ts`; `npx tsx scripts/calls-api-check.ts`; open /calls, toggle the Dialer chips.
**Result:** table total 7,502 → **7,500** after removing the 2 duplicates. Brianne = 65% of all dialing over 30 days (1,566 of 2,394).

### [2026-08-12] Call import automated — GHL `messages/export` replaces the manual CSV upload
**Status:** **VERIFIED.** Fixtures **30/30** suites green (new `calls-api-check` 38; `calls-live-check` 24 → **26 and back to GREEN** — it was 18/24 before this work on stale baselines). `tsc` = the same **7** pre-existing errors, none in a touched file. `next build` ✓. Deployed `lumin-deals-85dtndkvn` ● Ready. First real run imported **113 dials**; replaying the window inserted **0** (117 duplicates).
**Issue:** Efrain: *"automate the call import with that export endpoint."* /calls depended on someone remembering to download two GHL CSVs and upload them.
**Research:** `docs/research/2026-08-12-ghl-call-api-ingest.md` — every figure below was probed live against BOTH sub-accounts before any code was written.

**⚠️ The finding that would have corrupted the page: `TYPE_CAMPAIGN_VOICEMAIL` is not in the CSV.** `channel=Call` returns ringless voicemail drops next to real dials — **615 of them in a 5-day window** against 713 real calls. Importing them inflates dial counts ~45% and wrecks dials/lead. Only `TYPE_CALL` is a dial; fixture-locked as the first assertion in `calls-api-check`.
**The 1:1 proof:** API `TYPE_CALL` for 2026-08-04→09 returned **713**; the table holds **713** CSV rows for that window. Exact.
**`meta.call.duration` is SECONDS** — on all **393** calls where both sources report non-zero, db/api ratio is **1.000 at p10, p50 AND p90**. Assuming ms would have divided every talk-time metric by 1000.
**`from`/`to` + `direction` are reliable** — dialer matched **710/710**, direction **712/712**. So `account_label` is now DERIVED from the queried location instead of hand-tagged at upload (the CSV couldn't: Brianne's number appears in both exports).
**No migration.** API `dateAdded` carries ms, the CSV truncates to the second; truncating makes the row collide with its CSV twin on the EXISTING `(call_ts, contact_phone, dialer_number_phone)` index. `source_file='ghl-api'` marks provenance and makes the import precisely reversible.

**⚠️ UNRESOLVED AND DELIBERATELY NOT ASSERTED — the two sources disagree on 27% of individual calls.** 189 of 712 paired rows disagree on whether the call connected at all, in BOTH directions (105 API>0/CSV=0, 84 the reverse, incl. a 4,839s CSV row the API calls 0); where both report a duration they agree exactly. I tested and **rejected** the obvious explanation (Brianne dialing into both sub-accounts creating cross-account mispairs) — no call appears under both labels in that window, no duplicate API rows, and same-account-only pairing reproduces the identical split. Which source is right is **not determinable from our side**, so nothing claims it is.
**The aggregates are unaffected, measured:** like-for-like outbound connect rate is **69% on both** sides (CSV Aug 1–10 n=948 median 7s mean 52s; API Aug 11–12 n=98 median 6s mean 46s).
**Therefore FORWARD-ONLY:** each account resumes from its own newest stored call and never revisits the CSV period — because re-importing history would both rewrite what connect rate means for months of data AND duplicate calls (the truncated second lands ±1s from the CSV's on **113 of 711** rows, slipping past the dedupe index). Do not add a backfill mode without solving both.
**Changes:** NEW [lib/callsApi.ts](lib/callsApi.ts) (fetch + pure mapper), NEW [lib/callsSync.ts](lib/callsSync.ts) (forward-only sweep, per-account watermark, name resolution from `deals`, chunked idempotent upsert), NEW [app/api/sync/calls/route.ts](app/api/sync/calls/route.ts) (`?dry=1`, `?since=`), 30-min throttle in [app/api/cron/ghl-sync/route.ts](app/api/cron/ghl-sync/route.ts) (`calls_sync_last`), NEW [scripts/calls-api-check.ts](scripts/calls-api-check.ts).
**⚠️ Also fixed two now-invalid live assertions.** `calls-live-check` asserted an exact table total, which the sweep makes permanently false; it now asserts the CSV-imported portion never shrinks and the total grows only via API rows. That count was **already** drifting before this work (7,353, not the 7,346 baseline — a later manual CSV upload), so an exact-equality check there was red for a reason unrelated to this feature. Lead-count bands widened: the sweep now advances the call window's right edge every 30 min, on top of the 15-min `deals` sync adding leads inside it.
**⚠️ FOLLOW-UP SAME DAY — caught by Efrain's question, not by a test: the dialer's NAME was missing.** He asked *"will this be useful if I want to find out how many calls Brianne has made in each of the accounts?"* The export carries the dialing NUMBER but not its GHL label, and `effortRollup`/`dialerRollup` group on `dialer_number_name` with an **'Unknown'** default — so every automated call was about to disappear from per-dialer reporting, silently. Fixed (`lumin-deals-qfcud9u77`): `buildDialerNameMap` learns phone→label from rows already labelled (most-frequent wins — "Efrain's Number" and "Efrain" both exist), resolving **131/131** live; the 131 rows imported before the fix were backfilled null-only. A number that matches nothing is reported via `unlabelledDialers` instead of being silently bucketed. **Brianne dials from a DIFFERENT number in each sub-account** (…5677 Moe 2,429 calls / …8630 Matt 2,056) — that is the only reason her per-account split is answerable, so the resolver keys on the number, never the person. `calls-api-check` 38 → **41**.
**⚠️ Also made the never-dialed-spend assertions two-sided.** They asserted that number only ever grows — true only while call data was a frozen snapshot. The sweep can now dial a previously-never-dialed lead and pull it DOWN; Matt's fell **$762 → $737** on day one, which surfaced as a red check that was actually the feature working.
**Test Method:** `npx tsx scripts/calls-api-check.ts`; `npx tsx scripts/calls-live-check.ts`; `GET /api/sync/calls?dry=1` to inspect without writing.
**Result:** Moe 54 + Matt 59 = 113 dials imported (Aug 11 01:23 → Aug 12 15:09), 293 messages fetched → 113 after the TYPE_CALL filter, names resolved 108/113, statuses completed 92 / no-answer 15 / voicemail 2 / failed 4. Randy correctly skipped. **The manual CSV importer still works and was left in place** — it is the only source of `Disposition` (verified: no metric reads it).

### [2026-08-11] GHL capability audit → status webhook live + NoteCreate branch made reachable
**Status:** **PARTLY VERIFIED.** The status workflow is **VERIFIED LIVE**; the note fix is **CHANGED** (fixtures + live query pass; awaiting a real GHL note to confirm end-to-end).
**Issue:** Efrain: *"Check the GHL API and webhook capability… see if I am getting everything that is useful or if I am missing anything."*

**VERIFIED — real-time lost/abandoned coverage.** Efrain built an **Opportunity Status Changed** workflow in Matt's sub-account (two triggers: Moved-to *Lost* + Moved-to *Abandoned*) pointing at the existing webhook URL with `contactId={{contact.id}}`. Live test: **Damon Hunnicutt** flipped to Lost with his stage untouched → `stage_events` row `Attempted Contact → (lost)`, `source='webhook_status'`, logged **264ms** after the flip; deal demoted to `Not Ready`, `ghl_status='lost'`, stage label preserved. This closes the gap where a status-only flip waited up to ~3h for the maintenance scan (indefinitely off-hours). Moe's account: workflow published, **not yet exercised** — no `webhook_status` event from that location yet.
⚠️ The trigger **requires** a status filter and its value picker is **single-select**, so it needs TWO triggers; stacking `Moved to` + `Moved from` filters ANDs them and would only fire on Open→Lost.
⚠️ Fires only on moves **into** lost/abandoned — an un-lose still waits for maintenance or a manual Sync GHL.

**CHANGED — the NoteCreate branch could never have worked from a workflow.** It read note text from top-level keys only, but a Workflow webhook can only send it as Custom Data, which arrives **nested under `customData`** — the identical nesting that kept the message branch dead until `74e2aef`. Every workflow-sent note would have been rejected as "No note content".
⚠️ Second defect, worse: the branch resolved its deal with `.single()`, which **errors** when a contact owns several deals. Measured live: **479 contacts own >1 deal** — roughly one in six — so notes would have been silently dropped for exactly the multi-loan borrowers they matter most for. Newest deal now wins (same rule `buildContactDealMap` uses for GHL tasks).
Also: bare `id` is only trusted when the payload is **not** an opportunity (polymorphic-id guard — a note fired on a contact who owns an opportunity carries the OPP id there), and an **unresolved merge tag is rejected** rather than written into `lo_notes` (`pipelineStageName` came back unresolved on 146/146 audited bodies, so `{{note.body}}` mis-mapping is a live risk).
**Changes:** `noteText()` added to [lib/webhookPayload.ts](lib/webhookPayload.ts); note branch rewritten in [app/api/webhooks/ghl/route.ts](app/api/webhooks/ghl/route.ts); [scripts/webhook-fields-check.ts](scripts/webhook-fields-check.ts) 32 → **45** assertions; click-paths in [docs/runbooks/ghl-webhook-workflows.md](docs/runbooks/ghl-webhook-workflows.md).
**Test Method:** add the **Note Added** workflow (runbook Part 3), type a note on a test contact in GHL, confirm it appears on that deal within ~1 min. Fixtures: `npx tsx scripts/webhook-fields-check.ts`.
**Result:** fixtures **45/45**; `tsc` = the same **7** pre-existing errors, none in a touched file; `next build` ✓; deployed `lumin-deals-crasp2foy` ● Ready. Full suite **28/29** — `calls-live-check` 18/24 fails on **pre-existing baseline drift**, proven by stashing this change and re-running (identical 18/24): never-dialed spend rose ($2,079→$2,520 Moe, $762→$1,272 Matt) as new purchased leads land inside the frozen call window, and Lending Tree drifted $47→$49.79/connect. Not a regression from this work.
**⚠️ The Note Added workflow was DECLINED by Efrain the same day** (*"I dont think I need the note workflow either"*) — the handler fix stays because both defects were real and would also break a native `NoteCreate` event, but the branch is **dormant, by decision**. Do not re-propose it, and do not read the dormant code as unfinished work.
**Audit findings not built (Efrain's calls):** appointments — **declined**, don't re-propose. Outbound **texts/emails** have **no GHL workflow trigger at all** (open feature request), so workflow-able outbound is **calls only**. Still unread and available: `attributions` (utmAdId/utmCampaign/adSource) already stored on every synced opp — per-ad ROI with no new API calls; the `Latest Transaction *` property-records fields incl. the borrower's current interest rate; `lumin_lead_id` (2,615 filled, **zero readers**). `GET /conversations/messages/export` **works with the existing keys** (probed: 39.5k/35.7k messages, call rows carry duration + status) — the manual call-CSV import could be automated, losing only the hand-tagged Disposition.

### [2026-08-10] /calls — Activity tab, date filtering, best-time-to-call
**Status:** **VERIFIED.** Fixtures **75/75** (was 57 — 18 new cover PT bucketing), tsc clean for all call-report files, `next build` ✓, every preset exercised in the browser, no console errors, deployed.
**Issue:** Efrain: *"Get rid of these sections. Also give me the ability to filter the calls so I can see how many calls were made today, this week, this month, or custom range."*
**Removed:** the "Never dialed" and "Dialed, never connected" rows from the Effort tab. The rollups still COMPUTE them (`neverDialed`, `neverDialedSpend`, `dialedNeverConnected`) — they're asserted by `calls-live-check.ts` as a regression guard on the phone→deal join, and remain available if the lists are ever wanted elsewhere. Only the UI blocks are gone.
**⚠️ The date filter could NOT be applied to the whole page.** Effort's figures are per-lead LIFETIME stats (% dialed, dials/lead). Filter those to "today" and a May lead dialed in May reads as never dialed. Efrain chose activity-only scoping: a new **Activity** tab owns the filter (Today / This week / This month / Last 30 days / All time / Custom), while Effort and Economics stay on the full imported window and now say so on the page.
**⚠️ Bucketing is by PACIFIC calendar day, never stored UTC.** A 3:29pm PT call is stored `22:29Z`; bucketing raw UTC would file it at 10pm and shift "best time to call" by the whole offset — and a 6:30pm PT call stores as the NEXT UTC day, so a naive `slice(0,10)` moves it a day. `ptParts()` in [lib/callsCsv.ts](lib/callsCsv.ts) handles both; fixtures cover the 6:30pm case explicitly.
**Design note:** activity is returned **pre-bucketed per PT day** (~91 rows) rather than as raw calls, so every range — including custom — recomputes instantly client-side with no refetch, on a small payload. Deliberately **no distinct-contact count**: a distinct count can't be summed across daily buckets (a lead called Mon and Tue would double-count), and a silently-wrong "leads touched" is worse than no metric.
**Also:** hour/weekday analysis is **outbound only** (inbound connects by definition and washes out the signal) and hides buckets under 5 dials (one lucky call isn't a pattern). Avg conversation divides talk time by CONNECTED calls, not all calls, so voicemails don't drag it down. Daily chart switches from paired bars to areas past 40 days — at 91 days the bars collapse to invisible 3px slivers.
**Changes:** `ptParts()` + `activityBuckets`/`activityInRange`/`byHourOfDay`/`byWeekday`/`dialersInRange` in [lib/callsReport.ts](lib/callsReport.ts); `activity` added to [app/api/calls/route.ts](app/api/calls/route.ts); [app/calls/page.tsx](app/calls/page.tsx) rebuilt with 3 tabs; 18 new fixtures in [scripts/calls-check.ts](scripts/calls-check.ts).
**Test Method:** `npx tsx scripts/calls-check.ts`; exercise each preset on /calls.
**Result:** All time — 7,346 calls · 5,131 connected · **70%** · 168.7h talk · avg conversation 1m58s · 81 calls/active day · outbound 6,449 / inbound 897 · **best hour 7a (83%)**. Today (Aug 10) — 151 calls · 77% · best hour 9a (89%). Weekday connect rates sit in a tight 70–72% Mon–Fri (Fri 70%, Sat 77% on only 26 calls). Per dialer, all-time: Mohammad's number averages **6m23s** per conversation vs Brianne's **0m33s** — different jobs, not different effort.

### [2026-08-10] Call Report import (/calls + /import/calls) — GHL call CSV joined to purchased leads
**Status:** **VERIFIED.** Efrain ran `supabase-calls.sql`; table shape, unique index (dup insert → 23505) and anon-read block all confirmed live. Imported **7,346 calls** (7,348 parsed − 2 collapsed duplicates), re-import added **0**. `scripts/calls-live-check.ts` **24/24**, fixtures **57/57**, tsc clean (no new errors), `next build` ✓, both tabs rendered and screenshotted in the browser with no console errors, deployed.
**Issue:** Efrain: *"Are you able to pull data from the call report to the dashboard?"* GHL's Call report can't join dialing activity to lead source, lead price, or funded outcome.
**⚠️ GHL has NO call API — verified live, not assumed.** `/calls`, `/reporting/calls`, `/phone-system/calls`, `/voice-ai/call-logs` all 404 against a Private Integration token. Per-conversation `TYPE_CALL` messages exist and carry duration + status, but **no Disposition** (the LO's hand-tag). The CSV export is therefore both richer and the only practical ingest path. Do not re-investigate an API sync.
**⚠️ "Answered" does NOT mean a human picked up.** 724 rows in the real export are simultaneously `call_status='Answered'` AND `disposition='No Answer / Voicemail'` — a carrier-level connect. **The connect signal is `duration_sec > 0`, never `call_status`.** Trusting the status inflates connect rate by ~20 points. Encoded once in `isConnected()` in [lib/callsReport.ts](lib/callsReport.ts).
**⚠️ CSV timestamps are account-local PT, not UTC.** Parsing them as UTC produced dials landing ~7h BEFORE their own lead-in date — that artifact is the tell. `ptToUtc()` derives the offset from the instant via `Intl`, so PST (−8) and PDT (−7) both work; **a hardcoded −7 would silently shift every winter import by an hour** and is fixture-guarded by a January case.
**⚠️ The dialer is not the lead owner.** "Brianne's Number" appears in BOTH sub-account exports (2,427 calls in Moe's, 2,056 in Matt's), so the dialing number identifies neither the account nor the LO. `account_label` is tagged by the user at upload because it cannot be derived. Dialer is stored; **lead owner is always derived from the `deals` join at read time** so it can't drift the way `date_added_ghl` did.
**⚠️ False zeros are the failure mode.** An LO with no imported export has no evidence either way and renders **"no data", never 0%** (`coveredLos()`), and leads are scoped to the window the imported calls actually cover — otherwise an older lead reads as "never dialed" purely because no calls exist for that period. Randy is excluded by Efrain's decision (662 leads / $22,503 unrepresented).
**Changes:** NEW [supabase-calls.sql](supabase-calls.sql) (table, dedupe unique index, RLS-on-no-policies server-only like `stage_events`), [lib/callsCsv.ts](lib/callsCsv.ts) (parser + PT→UTC + duration), [lib/callsReport.ts](lib/callsReport.ts) (effort/dialer/economics rollups), [app/api/import/calls/route.ts](app/api/import/calls/route.ts) (preview + idempotent upsert), [app/import/calls/page.tsx](app/import/calls/page.tsx), [app/api/calls/route.ts](app/api/calls/route.ts), [app/calls/page.tsx](app/calls/page.tsx), [scripts/calls-check.ts](scripts/calls-check.ts) (57 fixtures), [scripts/calls-live-check.ts](scripts/calls-live-check.ts); Sidebar gains Call Report + Import Calls.
**Test Method:** 1) run `supabase-calls.sql` in Supabase; 2) import both exports at `/import/calls` tagged Moe / Matt; 3) `npx tsx scripts/calls-live-check.ts`.
**Result:** **24/24 live assertions pass.** Live page: Moe 718 leads/$26,970 · 92% dialed · 87% connected · 4.2 dials/lead · 71.8 talk-h · 20 min to first dial · 54 never dialed ($2,104) · 40 dialed-never-connected ($1,541). Matt 626/$20,619 · 96% · 89% · 4.9 dials/lead · 38.8 talk-h · 27 min · 25 never dialed ($762) · 44 dialed-never-connected ($1,309). Economics: LMB $38/connect (5 funded), OwnUp $80 (2), Lending Tree $47 (2), Lendgo $26 (2), FRU $32 (2).
**⚠️ Two acceptance criteria were WRONG as written and were corrected, not relaxed:**
1. *"No dial precedes its lead-in date."* False — 11 contacts (72 calls) precede by 23h–86 days. Diagnosed before changing anything: **zero** fall in the 1–9h band where a timezone shift would land, 10 of the 11 are non-purchased ($0), and one is a lender contact ("Linda The Loan Store"), not a borrower. That is `date_added_ghl` drift from GHL contact re-creation. The assertion now targets the 1–9h timezone signature and reports drift separately.
2. *Exact lead counts.* `deals` is live (15-min sync), so new purchased leads keep landing inside the frozen call window and arrive "never dialed" — Moe moved 717→718 and $2,079→$2,104 within the hour. Counts assert "≥ baseline within a band"; the ratios stay tight.
**Also corrected:** row count is **7,346**, not 7,348 — there are TWO byte-identical duplicate pairs (one in each export), not one in Moe's alone.
...
**⚠️ Purchased-only is evidence-backed, not a preference.** Anchor coverage measured live (bucketed with the app's own `isPurchased`, after a first pass with a hand-typed source list produced a wrong denominator by omitting Lending Tree): agg leads **3,006/3,008 (99.9%)**, **$95,733 of $95,832** spend, **41/42 funded**; warm/organic carries it on only **47 of 136 funded loans (34.6%)**. A cohort built from warm sources would silently under-report funding, so it's excluded and the excluded count is shown next to the period picker rather than folded into a denominator.
...
### [2026-08-24] Data: Brianne's two rotated dialing numbers labelled (327+ calls rescued from "Unknown")
**Status:** VERIFIED
**Issue:** Efrain — "I have changed Brianne's number on both profiles." `dialer_number_name` is resolved ONLY by phone number, from a map `buildDialerNameMap()` learns from already-labelled rows (`lib/callsSync.ts`). A rotated line has no labelled row to learn from → resolves null → every call falls into 'Unknown' and drops out of the per-dialer views that are the whole point of /calls. Silent: no error, and `runCallsSync`'s `unlabelledDialers` counter is returned but read by nothing.
**Root cause (structural, still open):** the learned-map design assumed CSV imports would keep supplying labels. Ingest went API-only + forward-only 2026-08-12 and the export payload carries the dialing number WITHOUT its GHL title, so no new label can ever enter the system on its own. Every future rotation repeats this.
**Changes:** `scripts/dialer-label-backfill.ts` (new) — sets `dialer_number_name = "Brianne's Number"` on `9497732190` (moe) and `9497389920` (matt) where currently null. Same label as her old lines on purpose: keeps her volume continuous across the rotation instead of splitting her into two dialers; `accountsForDialer` still splits moe/matt underneath. No app code changed, so no deploy.
**Test Method:** full-table dialer audit before/after (number × label × volume × first/last seen × account); daily per-number volume Aug 14–24 to confirm the cutover; `--dry` run; `scripts/calls-live-check.ts`.
**Result:** Cutover confirmed at 2026-08-21 — old lines 107/2 calls on Aug 19 → 7/3 on Aug 24, while the new pair went 0/0 → 110/72. Backfilled **199 (moe) + 135 (matt) = 334 rows**. All four of Brianne's numbers now read "Brianne's Number" (3,100 + 2,415 + 199 + 135 = 5,849 calls). Unlabelled rows 340 → **13**, and those 13 are the blank-`dialer_number_phone` settle-window ghosts (2026-08-12 onward), a separate issue.
**Identity check:** the two numbers were confirmed as Brianne's by Efrain directly. Inferred first from the cutover shape, the one-number-per-sub-account pattern unique to her, the 949 area code and a 97% outbound mix — GHL's own `/phone-system/numbers` titles could not be read (`.env.local` access blocked).
**calls-live-check 23/26 — NOT caused by this change:** Moe dials/lead 4.72 (want ~4.2), Matt 5.15 (want ~4.9), LendingTree $50.41/connect (want ~$47). Those baselines are frozen to the original May 4 – Aug 10 export window while the 30-min sweep keeps adding dials against the same lead cohort. Verified mechanically that the label cannot move them: `dialer_number_name` is read at only two places in `lib/callsReport.ts` (129, 286), both sub-breakdown keys — dials/lead and cost/connect count per `contact_phone` and never touch it.

### [2026-08-25] Feature: dialer names resolve from GHL, so a rotated number names itself
**Status:** VERIFIED
**Issue:** Follow-on from the 2026-08-24 backfill. `dialer_number_name` was learned ONLY from rows already stored, and the messages/export payload carries the dialing number without its title — so once ingest went API-only + forward-only (2026-08-12) no new number's name could ever enter the system. Every future rotation would silently repeat Brianne's 334 calls landing in 'Unknown'. `unlabelledDialers` already counted it; nothing read it.
**Changes:** `fetchNumberLabels(acct)` in `lib/callsApi.ts` — GETs `/phone-system/numbers` per sub-account, returns normPhone→title, and returns an EMPTY map on any failure so a naming lookup can never fail a sweep. `lib/callsSync.ts`: `buildDialerNameMap` now returns `DialerLabels { byPhone, canonical }` (canonical = lower-cased label → the spelling the stored rows use most); new `resolveDialerName(phone, ghlTitles, labels)` asks GHL first, folds its title onto the stored spelling, and falls back to the learned per-number map; one GHL request per account per sweep, fetched lazily only when there are rows to name. New `ghlNamedNumbers` on the per-account result as the health signal for the path. `scripts/dialer-labels-check.ts` (new, read-only) diffs GHL titles against stored labels.
**⚠️ The trap this nearly shipped with:** GHL titles both of Brianne's numbers **"Brianne's number"** (lower-case n) while all 5,849 of her stored calls read **"Brianne's Number"**. The rollups group on the exact string, so taking GHL's spelling verbatim would have split her into two dialers on the page — the same bug in a new costume. Caught by the probe BEFORE shipping, hence the canonical fold. Also confirmed: her two retired lines are already absent from GHL's list while 5,515 of their calls remain, so the learned map is a permanent fallback, not a legacy path.
**Test Method:** `scripts/dialer-labels-check.ts` against both live sub-accounts; 8 new assertions in `scripts/calls-api-check.ts` (new number named from GHL, spelling folded, case-insensitive, retired number falls back, GHL unreachable degrades to prior behaviour, unrecognised name used as-is, blank dialer stays unnamed); `calls-api-check` 52/52; `calls-check` 92/92; `tsc` (7 pre-existing only, none in touched files); `next build`; live `runCallsSync` dry run over 2026-08-22→now.
**Result:** Dry run `ok`, writes nothing, `ghlNamedNumbers: 3` on BOTH accounts. `unlabelledDialers` for the window: moe **1**, matt **0**.
**New finding — the 13 blank-dialer rows are NOT all settle-window ghosts:** chasing moe's last unlabelled row found GHL returning `from: "Moe Sefati"` — the user's DISPLAY NAME where the number belongs. `normPhone` can't parse it, so `dialer_number_phone` becomes `''` and the row correctly stays unnamed. Distinct from the 2026-08-12 incident (which was `from`/`to` EMPTY) and not fixed by `SETTLE_MS`. 13 rows since 08-12, ~1/day. Deliberately NOT auto-named: `"Moe Sefati"` is a person, not a line, and inventing a dialer bucket from it would mix two different things on the page. Left for a decision.

### [2026-08-25] Fix: the 'Unknown' dialer bucket emptied — two causes, two repairs
**Status:** VERIFIED
**Issue:** Efrain — "fix the name-in-the-number-field rows too." 13 stored calls had `dialer_number_phone = ''` and no dialer name, so they sat in the per-dialer 'Unknown' bucket.
**Root cause — NOT one bug, two:**
  **A · Name in the number field (10 rows).** GHL rejects an invalid destination BEFORE assigning an outbound line and substitutes the dialing user's display name: `status: 'failed'`, `error: 'VOICE_CALL_INVALID_PHONE_NUMBER'`, `from: "Moe Sefati"`. Not corrupt — the name is the truth about who dialed, only the number is absent. All 10 were Moe, and 9 of them to the SAME invalid contact number (8612106747).
  **B · Settle-window ghosts (3 rows).** Read before GHL had populated `from`/`to`. ⚠️ `SETTLE_MS` (5 min, shipped 2026-08-12) reduced but has NOT eliminated these — two are from 08-19. GHL now returns all three complete, and exactly one stored row exists per call (no twin), so number and label were both recoverable.
**Changes:** `mapApiCall` passes an unparseable dialer field through as a label hint (`dialerNameHint`); `resolveDialerName` gains a `nameHint` arm that folds onto an existing spelling like a GHL title does. ⚠️ `dialer_number_phone` stays `''` for class A — it is one third of `calls_dedupe_uniq` and deriving a number would re-key stored rows and re-insert them all. `scripts/blank-dialer-backfill.ts` (new) repairs both classes, and every class-B re-key is guarded against a row already holding the target key.
**Test Method:** live payload probe of all odd rows across both sub-accounts; per-row DB+GHL comparison of the 3 ghosts to prove no twin existed; `--dry` run; 9 new assertions in `calls-api-check` (person named, dedupe key unchanged, contact still from `to`, failed call still a dial, name folds, person NOT merged into a line, empty stays unnamed, whitespace is not a name); `calls-api-check` 61/61; `calls-check` 92/92; `tsc` (7 pre-existing, none in touched files); `next build`.
**Result:** named 10 · number restored 3 · skipped 0 · unmatched 0. Full-table audit: **unlabelled rows 13 → 0**. The 'Unknown' dialer bucket no longer exists. Restored rows joined their existing buckets (Matthew's number ×2, Brianne's Number ×1); the 10 name-only rows form a "Moe Sefati" bucket, deliberately separate from "Mohammad's number".
**Live confirmation of the 08-25 GHL naming deploy:** during this session the sweep carried Brianne's new numbers 199→205 and 135→138 with labels applied automatically and zero unlabelled — the self-healing path working in production, not just in a dry run.
**Worth a look, not a defect:** 9 failed dials to contact 8612106747 — the stored number is invalid and GHL refuses it. Someone is repeatedly calling a number that cannot connect.

### [2026-08-25] Feature: "Numbers that can't be called" on /calls
**Status:** VERIFIED
**Issue:** Efrain — after Nestor Morris's Lendgo number was refused by the carrier 10 times before anyone noticed: "add that check." A refused dial is indistinguishable from a lead who won't pick up unless someone reads the raw status, so a dead number burns dials indefinitely.
**Changes:** `unreachableRollup(calls, deals)` + `isCarrierFailure` + `UNREACHABLE_MIN_FAILURES = 3` in `lib/callsReport.ts`; returned as `unreachable` from `/api/calls` (full window, never date-filtered — a dead number is dead in every range); amber panel on the **Effort** tab of `/calls` listing lead, number, refused-of-total, source, lead cost and last attempt, linking to the deal.
**⚠️ The gate that keeps it readable:** flagged on FAILED status, NOT on "never connected". Hundreds of ordinary leads have zero connects and are normal follow-up; a 'failed' means GHL rejected the number before placing the call. Mixing them would bury ~10 real rows in hundreds and the panel would go unread. Connects are still measured by `isConnected()` (duration > 0), so an 'Answered' voicemail does NOT clear a number — asserted directly.
