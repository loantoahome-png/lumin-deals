...
**A. loan_purpose overlay** — [lib/ghlOpportunityFields.ts](lib/ghlOpportunityFields.ts) now maps the opportunity's "Loan Purpose" through `normalizeLoanPurpose`. The sync already overlays opp fields on top of contact defaults (`route.ts:968`), so the opportunity wins and the contact stays the fallback — no call-site change needed. Fixes the last gap: the contacts LIST endpoint omits custom fields, so purposes GHL held on the opportunity were never read (16 of Moe's agg leads were HELOC in GHL and blank here — his entire untagged bucket).
