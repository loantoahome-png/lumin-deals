...
**⚠️ NOT APPLIED — needs Efrain:** `npx tsx scripts/loan-purpose-backfill.ts "<contacts export>.csv" apply`. Deals only self-heal when the sync next re-reads that contact WITH custom fields (the per-contact GET path); a dormant lead can wait indefinitely. **Second pending backfill from earlier today:** `scripts/arive-source-backfill.ts apply` (200 rows).
...
**⚠️ BACKFILL NOT APPLIED** — the 200-row bulk write was blocked by the permission classifier and was deliberately not worked around. Dry run is verified and repeatable; Efrain runs `BACKFILL_STAMP=2026-07-28 npx tsx scripts/arive-source-backfill.ts apply` (backs up before-state first). Until then the 200 keep the wrong label, but the guard means **no new drift** and any deal whose GHL opportunity carries a real source now self-heals on the next full maintenance pass (~3h) — Garry Swatzel is one of those 13 (`opp.source='FRU'`), so he should flip to FRU without the backfill.
