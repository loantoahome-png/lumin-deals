...
export async function POST(req: NextRequest) {
  try {
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // TASK EVENTS — mirror GHL tasks onto the dashboard (deal_tasks), read-only
    //   GHL fires TaskCreate / TaskComplete / TaskUpdate / TaskDelete. We key the
    //   mirrored row by the GHL task id so repeat events update in place; the
    //   dashboard shows them but completion/edits stay driven by GHL. Requires
    //   supabase-ghl-tasks.sql to have been run (adds ghl_task_id / ghl_contact_id
    //   / source columns) — until then the upsert fails softly and is logged.
    // ══════════════════════════════════════════════════════════════════════════
    if (eventType.toLowerCase().includes('task')) {
      const ev = eventType.toLowerCase()
      const taskId = pick(body, 'id', 'taskId', 'task_id')
      if (!taskId) return NextResponse.json({ success: false, reason: 'task event with no task id' })

      // Delete events drop the mirrored row (only ever touches GHL-sourced tasks).
      if (ev.includes('delet')) {
        await supabase.from('deal_tasks').delete().eq('ghl_task_id', taskId).eq('source', 'ghl')
        console.log('[GHL Webhook] Task deleted:', taskId)
        return NextResponse.json({ success: true, action: 'task_deleted', taskId })
      }

      // Link to the contact's dashboard deal (most-recently-updated if several).
      const taskContactId = pick(body, 'contactId', 'contact_id')
      let dealId: string | null = null
      if (taskContactId) {
        const { data: dRow } = await supabase
          .from('deals').select('id').eq('ghl_contact_id', taskContactId)
          .order('updated_at', { ascending: false }).limit(1).maybeSingle()
        dealId = (dRow?.id as string | undefined) ?? null
      }

      // Resolve assignee: GHL sends a user id; map it to a name via the cache the
      // main sync maintains (ghl_user_map_cache:<locationId>).
      const assignedTo = pick(body, 'assignedTo', 'assigned_to', 'userId', 'user_id')
      const taskLocationId = pick(body, 'locationId', 'location_id', 'location')
      let assignee: string | null = pick(body, 'assignedToName', 'assignedUserName')
      if (!assignee && assignedTo) {
        let entries: Record<string, string> = {}
        if (taskLocationId) {
          const { data: c } = await supabase.from('sync_state').select('value').eq('key', `ghl_user_map_cache:${taskLocationId}`).maybeSingle()
          entries = (c?.value as { entries?: Record<string, string> } | null)?.entries ?? {}
        } else {
          const { data: cs } = await supabase.from('sync_state').select('value').like('key', 'ghl_user_map_cache:%')
          for (const r of cs ?? []) Object.assign(entries, (r.value as { entries?: Record<string, string> } | null)?.entries ?? {})
        }
        assignee = entries[assignedTo] ?? null
      }

      // Completion: preserve the existing state when the payload doesn't say, so a
      // plain update event never accidentally un-completes (or re-stamps) a task.
      const { data: existingTask } = await supabase.from('deal_tasks').select('completed_at').eq('ghl_task_id', taskId).maybeSingle()
      const explicit = body.completed === true ? true : body.completed === false ? false : null
      const completedNow = ev.includes('complete') ? true : explicit
      const prevCompletedAt = (existingTask?.completed_at as string | null) ?? null
      const completed_at =
        completedNow === true ? (prevCompletedAt ?? new Date().toISOString()) :
        completedNow === false ? null :
        prevCompletedAt   // unknown → keep whatever we had

      const row = {
        deal_id: dealId,
        ghl_task_id: taskId,
        ghl_contact_id: taskContactId ?? null,
        source: 'ghl',
        title: pick(body, 'title', 'name') ?? '(untitled GHL task)',
        description: pick(body, 'body', 'description'),
        due_at: pick(body, 'dueDate', 'due_date', 'due'),
        assignee,
        assigned_by: 'GHL',
        priority: null,
        completed_at,
      }
      const { error: tErr } = await supabase.from('deal_tasks').upsert(row, { onConflict: 'ghl_task_id' })
      if (tErr) {
        console.error('[GHL Webhook] task upsert failed (did you run supabase-ghl-tasks.sql?):', tErr.message)
        return NextResponse.json({ success: false, reason: 'task upsert failed', detail: tErr.message })
      }
      console.log(`[GHL Webhook] Task mirrored: ${taskId} → deal ${dealId ?? 'unlinked'} (${completed_at ? 'done' : 'open'})`)
      return NextResponse.json({ success: true, action: 'task_mirrored', taskId, dealId })
    }
