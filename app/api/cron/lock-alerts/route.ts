...
import { ALERT_LOS, isAlertLo, alertLoEmail } from '@/lib/alertRecipients'
...
// Recipients live in lib/alertRecipients.ts — Moe and Matt only — so this cron and
// scripts/lock-alert-preview.ts cannot drift apart. See that file for why it is an
...
export async function GET(req: NextRequest) {
  ...
  for (const d of (deals ?? []) as DealRow[]) {
    ...
    const loEmail = alertLoEmail(d.loan_officer as string | null)
    if (!loEmail) {
