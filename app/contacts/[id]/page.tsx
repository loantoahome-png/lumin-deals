...
import { formatCurrency, formatDate, titleCase, dndLabel, dndSummary } from '@/lib/utils'
...
/** DND / last-contacted rolled up across the person's loans. If ANY loan blocks a
 *  channel, surface it — safer to over-warn than to text someone who opted out. */
function reachability(deals: Deal[]): { dndText: string | null; lastContacted: string | null } {
  const blocked = deals.find(d => dndSummary(d))
  const lastContacted = deals
    .map(d => d.last_contacted)
    .filter((v): v is string => !!v)
    .sort()
    .pop() ?? null
  return { dndText: blocked ? dndLabel(blocked) : null, lastContacted }
}
