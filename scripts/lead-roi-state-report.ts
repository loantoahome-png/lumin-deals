...
async function main() {
  ...
  for (const lo of los) {
    ...
    console.log(`     reconcile → leads ${ok(sum(r => r.n), top.total)} · spend ${ok(sum(r => r.spend), top.spend)} · net ${ok(sum(r => r.netProfit), top.netProfit)} · submitted ${ok(sum(r => r.submitted), top.submitted)}`)
