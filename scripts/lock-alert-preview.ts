...
//
// Pure read — sends nothing, writes nothing. Exists because this alert mails four LOs
// and was dead for months (it required the `locked` flag, which has no importer), so
// the blast radius needs to be checkable without triggering a send.
...
async function main() {
  ...
  const due = live.map(r => ({ r, d: lockDaysLeft(r.lock_expiration as string) }))
  ...
  for (const { r, d } of upcoming) console.log(`   ${String(d).padStart(2)}d  ${String(r.name).padEnd(26)} LO=${r.loan_officer}`)
