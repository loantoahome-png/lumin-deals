...
- `/deals` — Active Escrows. Three views off one toggle: **By Lender** (the DEFAULT since 2026-09-16 — the SAME cards stacked under one section per lender, biggest loan volume first, "No lender on file" last; read-only, no drag, because a drop would rewrite `investor` on a live escrow), **Tracker** (the drag-by-stage kanban, `components/EscrowTracker.tsx`), and **Table**. ⚠️ Only `/deals` defaults to the lender grouping — `/processing` renders the same component and keeps the stage board, so `EscrowTracker`'s own `groupBy` default stays `'stage'`. Lender identity is normalized in `lib/lenderGroup.ts` (fixtures: `scripts/lender-group-check.ts`, 44) — `deals.investor` is free text with real drift (ROCKET/Rocket/Rocket Pro TPO/Rocket - CORE are one lender; Figure is split 5 ways), so grouping on the raw string would print one lender as four sections. Two maps, both in that file: `lenderKey()` merges spellings for DISPLAY (fuzzy), `canonicalLenderName()` is the EXACT-match spelling that gets WRITTEN (used by the Arive import, the GHL sync and the webhook, so the table can't re-drift) — a fixture asserts the two agree. The 60 stored values were collapsed to 45 on 2026-09-16 by `scripts/lender-name-cleanup.ts` (dry-run by default; `--apply` backs up to `_lender-cleanup-backup-*.json` first, `--revert=<file>` undoes it). ⚠️ Randy's ` - CORE` / ` Pro TPO` values are deliberately NOT normalized — all 71 are in his sub-account, July 2026, no Arive file, `broker_corr` null, so the suffix is his own label, not the channel; Efrain's call was to leave them until someone says what CORE means. Live read-out: `npx tsx scripts/lender-view-report.ts`.
