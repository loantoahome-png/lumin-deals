...
**What works:** `?full=1` (the sidebar's Full Sync) is currently the ONLY recovery path. The real fix is to stop the cursor from gating CREATION on maintenance runs: those already hold the complete opportunity list in memory, so an opportunity with **no existing deal** should be processed regardless of its `updatedAt`. Cheap — it only adds work for opportunities that are genuinely missing — and it turns "lost until a human notices" into "self-heals within 3 hours". Not shipped as of 2026-07-28; it changes sync CPU, which is a watched cost.
**Project:** lumin-deals
**Date:** 2026-07-28
